...
@section('title', __('contents.create_new'))
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.contents.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="attachments">{{ __('contents.label.attachments') }}</label>
                            <input type="file" name="attachments[]" id="attachments" class="form-control-file" multiple>
                            <small class="form-text text-muted">{{ __('contents.helper.attachments') }}</small>
                        </div>
                                            ...
                                            @if ($user->id != Auth::id())
                                                <div class="form-check">
                                                    <input type="checkbox" name="share_view[]" value="{{ $user->id }}" id="view_{{ $user->id }}" class="form-check-input"
                                                        {{ old('share_view') && in_array($user->id, old('share_view')) ? 'checked' : '' }}>
                                            ...
                                            @if ($user->id != Auth::id())
                                                <div class="form-check">
                                                    <input type="checkbox" name="share_edit[]" value="{{ $user->id }}" id="edit_{{ $user->id }}" class="form-check-input"
                                                        {{ old('share_edit') && in_array($user->id, old('share_edit')) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">{{ __('contents.helper.tags') }}</small>
                        </div>

                        <button type="submit" class="btn btn-success">{{ __('contents.button.save') }}</button>
                        <a href="{{ route('admin.contents.index') }}" class="btn btn-secondary">{{ __('contents.button.cancel') }}</a>
                    </form>
                </div>
            </div>
        </div>
